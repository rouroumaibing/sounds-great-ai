...
export function VoicePanel() {
  ...
  const handleTestTTS = () => {
    setTestResult(t('settings.voicepanel.s1') + ' 🔊');
    setTimeout(() => setTestResult(''), 3000);
  };

  const handleTestSTT = () => {
    setTestResult(t('voice.sttTestResult'));
    setTimeout(() => setTestResult(''), 3000);
  };
  ...
  return (
    <div className="max-w-5xl mx-auto w-full space-y-6">
      <div className="border-b border-slate-800/80 pb-5">
        <h2 className="text-2xl font-bold text-slate-100">{t('settings.voice')}</h2>
        <p className="text-xs text-slate-400 mt-1">{t('voice.desc')}</p>
      </div>
      ...
      <div className="rounded-2xl bg-slate-900/60 border border-slate-800/80 p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <div className="text-xs font-bold text-slate-200">{t('voice.service')}</div>
              <div className="text-[11px] text-slate-500 mt-0.5">
                {enabled ? t('voice.running') : t('voice.disabled')}
              </div>
            </div>
          </div>
          <button onClick={() => setEnabled(!enabled)} className={clsx('relative w-10 h-5 rounded-full transition', enabled ? 'bg-indigo-500' : 'bg-slate-700')}>
      ...
      <div className="rounded-2xl bg-slate-900/60 border border-slate-800/80 p-4 space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <h4 className="text-xs font-bold text-slate-200">{t('voice.ttsConfig')}</h4>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label={t('voice.voice')} value={ttsVoice} onChange={setTtsVoice} options={['alloy', 'echo', 'fable', 'onyx', 'nova', 'shimmer']} />
          <Field label={t('voice.langCode')} value={ttsLang} onChange={setTtsLang} options={['zh-CN', 'en-US', 'ja-JP']} />
        </div>
        <div>
          <label className="text-[11px] text-slate-400">{t('voice.speed')} {ttsSpeed.toFixed(1)}x</label>
        ...
        <div>
          <label className="text-[11px] text-slate-400">{t('voice.refAudio')}</label>
      ...
      <div className="rounded-2xl bg-slate-900/60 border border-slate-800/80 p-4 space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <h4 className="text-xs font-bold text-slate-200">{t('voice.sttConfig')}</h4>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label={t('accounts.models')} value={sttModel} onChange={setSttModel} options={['whisper-1', 'whisper-large-v3']} />
          <Field label={t('voice.language')} value={sttLanguage} onChange={setSttLanguage} options={['zh', 'en', 'ja', 'auto']} />
        </div>
        <div className="flex items-center justify-between">
          <span className="text-[11px] text-slate-400">{t('voice.autoTranscribe')}</span>
      ...
      <div className="rounded-2xl bg-slate-900/60 border border-slate-800/80 p-4 space-y-3">
        <div className="flex items-center space-x-2">
          ...
          <h4 className="text-xs font-bold text-slate-200">{t('voice.glossary')}</h4>
        </div>
        ...
        <div className="flex items-center space-x-2 pt-2 border-t border-slate-800/40">
          <input type="text" value={newKey} onChange={(e) => setNewKey(e.target.value)} placeholder={t('voice.termPlaceholder')} className="flex-1 px-2 py-1.5 rounded-lg bg-slate-800/50 border border-slate-700/50 text-[11px] font-mono text-slate-200 focus:border-purple-500/50 transition" />
          <input type="text" value={newValue} onChange={(e) => setNewValue(e.target.value)} placeholder={t('voice.defPlaceholder')} className="flex-1 px-2 py-1.5 rounded-lg bg-slate-800/50 border border-slate-700/50 text-[11px] text-slate-200 focus:border-purple-500/50 transition" />

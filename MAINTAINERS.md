...
This guide is for **maintainers and triagers** of the Sounds Great AI project. If you're looking for how to contribute, see [CONTRIBUTING.md](CONTRIBUTING.md).
...
gh issue list --repo sounds-great-ai/sounds-great-ai --state open --search "{keywords}"
```

| Result | Action |
|--------|--------|
| Already tracked by a Feature | Add `feature:FT-XXX` label, comment linking the issue |
| Related but not identical | Add `feature:FT-XXX`, maintainer decides merge or separate |
| Completely new | Needs maintainer decision on scope and FT 编号 |
| Too small for a Feature | Keep as `enhancement`, no FT 编号 needed |
...
```markdown
Designating this as the tracking issue for FT-XXX.
...
**Child issue comment:**
```markdown
This issue is tracked under FT-XXX (umbrella: #{N}).
...
```bash
gh issue edit {N} --repo sounds-great-ai/sounds-great-ai --add-label "triaged"
```

If the issue needs a maintainer decision on direction or scope:

```bash
gh issue edit {N} --repo sounds-great-ai/sounds-great-ai --add-label "needs-maintainer-decision"
```
...
| `feature:FT-XXX` | Linked to a specific Feature (e.g. `feature:FT-113`) | Issue / PR |
...
| **Find your assignments** | `gh issue list --repo sounds-great-ai/sounds-great-ai --assignee @me` |
...
- Has `triaged` label
- Has a type label (`bug` / `enhancement` / `feature:FT-XXX`)
- State is OPEN
...
- [ ] **Quality** — CI passes, `go build/vet/test` + `tsc` clean, no security issues
...
- `sounds-great-ai#58` or `dog-pack#120` — unambiguous
...
```markdown
Shipped in FT-XXX (PR #{sync_pr}). Thank you for reporting!
```
...
Feature IDs (`FT-001`, `FT-002`, ...) are assigned by **senior maintainers only**. This is covered in [CONTRIBUTING.md](CONTRIBUTING.md#feature-numbering) — the rule is repeated here for completeness:

1. Contributor opens an Issue describing the feature
2. Maintainers discuss and approve
3. Senior maintainer assigns the next available FT 编号
4. Implementation proceeds against the Feature Doc's Acceptance Criteria

**Bug fixes don't get FT 编号s** — they're tracked by their GitHub Issue number.
...
3. Review the [current label list](https://github.com/sounds-great-ai/sounds-great-ai/labels) — familiarize yourself with what exists
...
本指南面向 Sounds Great AI 项目的 **maintainer 和 triager**。如果你想了解如何贡献代码，请看 [CONTRIBUTING.md](CONTRIBUTING.md)。
...
```bash
gh issue list --repo sounds-great-ai/sounds-great-ai --state open --search "{关键词}"
```

| 结果 | 处置 |
|------|------|
| 已有 Feature 覆盖 | 加 `feature:FT-XXX` 标签，评论互链 |
| 相关但不完全一样 | 加 `feature:FT-XXX`，maintainer 决定合并还是独立 |
| 全新需求 | 需 maintainer 拍板 scope 和 FT 编号 |
...
**总单评论：**
```markdown
把这张单定为 FT-XXX 的总跟踪 issue，聚合整体目标和进度。
...
**子单评论：**
```markdown
标记：这张单收敛为 FT-XXX 下的具体执行 issue，对应总单 #{总单号}。
...
| `feature:FT-XXX` | 关联到具体 Feature（如 `feature:FT-113`） | Issue / PR |
...
| **查自己的认领** | `gh issue list --repo sounds-great-ai/sounds-great-ai --assignee @me` |
...
- 有 `triaged` 标签
- 有类型标签（`bug` / `enhancement` / `feature:FT-XXX`）
- 状态为 OPEN
...
- [ ] **质量** — CI 通过，``go build/vet/test` + `tsc` 干净，无安全问题
...
Feature 编号（`FT-001`、`FT-002`、……）由**高级 maintainer 分配**。详见 [CONTRIBUTING.md](CONTRIBUTING.md#feature-numbering)，这里重述规则：

1. 贡献者开 Issue 描述功能
2. Maintainer 讨论并批准
3. 高级 maintainer 分配下一个可用 FT 编号
4. 按 Feature Doc 的验收标准实现

**Bug 修复不分配 FT 编号**——直接用 GitHub Issue 号追踪。
...
2. 设置仓库的 GitHub 通知
3. 看看[当前标签列表](https://github.com/sounds-great-ai/sounds-great-ai/labels)，熟悉现有标签

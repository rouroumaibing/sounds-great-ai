      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-lg p-6 space-y-4 shadow-2xl max-h-[90vh] overflow-y-auto" role="dialog" aria-modal="true">
        ...
        <div className="space-y-3 text-xs">
          <div className="grid grid-cols-2 gap-3">
            <Field label="账户标识 (name)"><input value={form.name} onChange={e => update({ name: e.target.value })} className={inputCls} /></Field>
          ...
          <div className="flex items-center justify-between p-3 rounded-xl bg-slate-950/60 border border-slate-800">
            <span className="text-slate-300">认证方式</span>
          ...
          <Field label="Base URL (可选)"><input value={form.baseUrl ?? ''} onChange={e => update({ baseUrl: e.target.value })} placeholder="https://..." className={inputCls} /></Field>
          ...
          <Field label="可用模型 (models)"><TagEditor tags={form.models ?? []} onChange={t => update({ models: t })} placeholder="model name" /></Field>

          <Field label="模型别名 (modelAliases)"><KeyValueEditor entries={form.modelAliases ?? {}} onChange={e => update({ modelAliases: e })} keyPlaceholder="alias" valuePlaceholder="real model" /></Field>

          <Field label="环境变量 (envVars)"><KeyValueEditor entries={form.envVars ?? {}} onChange={e => update({ envVars: e })} keyPlaceholder="ENV_VAR" valuePlaceholder="value" /></Field>
        </div>
